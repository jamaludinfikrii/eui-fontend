...
class ManageBrand extends Component {
    ...
    componentDidMount() {
        axios.get('https://eui-jamesfikrii-api.herokuapp.com/getlistbrand')
        .then((res) => {
    ...
    onBtnAddClick = () => {
        if(document.getElementById("AddBrandImage").files[0] !== undefined) {
            ...
            axios.post('https://eui-jamesfikrii-api.herokuapp.com/addbrand', formData, headers)
            .then((res) => {
    ...
    onBtnDeleteClick = (id) => {
        if(window.confirm('Are you sure to delete?')) {
            axios.delete('https://eui-jamesfikrii-api.herokuapp.com/deletebrand/' + id)
            .then((res) => {
    ...
    onBtnUpdateClick = (id) => {
        ...
        axios.put("https://eui-jamesfikrii-api.herokuapp.com/editbrand/" + id, formData, headers)
        .then((res) => {
        ...
        var listJSX = this.state.brandList.map((item) => {
            if(item.id === this.state.selectedEditBrandId) {
                return (
                    <tr>
                        <td></td>
                        <td><input type="text" ref="EditBrandName" defaultValue={item.nama} id='inputId'/></td>
            ...
            return (
                <tr>
                    <td>{item.id}</td>
                    <td>{item.nama}</td>
                    <td><img src={`https://eui-jamesfikrii-api.herokuapp.com/${item.image}`} alt={item.nama} width={100} /></td>
